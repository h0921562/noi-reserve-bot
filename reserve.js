...
async function getReservations() {
  ...
  $('table tbody tr').each(function() {
    ...
    if (cells.length >= 6) {
      reservations.push({
        password: $(cells[0]).text().trim(),
        date: $(cells[1]).text().trim(),
        time: $(cells[2]).text().trim(),
        tenant: $(cells[3]).text().trim(),
...
async function cancelReservation(date, startTime) {
  ...
  $('table tbody tr').each(function() {
    ...
    if (cells.length >= 6) {
      ...
      if (dateText.indexOf(date) >= 0 && timeText.indexOf(startTime) >= 0) {
        var link = $(this).find('a[href*="cancel"], a[href*="delete"]');
  ...
  p2.append('_token', newToken);
  try {
    await axios.post(cancelUrl.startsWith('http') ? cancelUrl : BASE_URL + cancelUrl, p2.toString(), {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', Cookie: getCookie() }
    });
    return { success: true };
  } catch (err) {
